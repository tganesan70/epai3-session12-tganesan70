# -*- coding: utf-8 -*-
"""session12_test_pynotes.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/16Vte0saOiiI85xMQu0Iqzu5A-_WQD2Wd
"""
...
"""# Polygon Iterator Class Defintion
...
"""# Tests for Polygon with iterable definition

"""

mypoly = Polygon(12, 1)
print(f'Polygon description: {mypoly.__repr__()}')
print(f'Polygon has {mypoly.count_edges} sides and the area is {mypoly.area}')

# Polygon Iterable class tests
mypoly = PolygonLazy(12, 1)
print(len(mypoly))
print(f'Polygon iterator description: {mypoly.__repr__()}')
print(f'Polygon iterator length is : {mypoly.__iter__().__len__()}')
for p in mypoly:
    print(p.count_vertices)
print(f'Print the area of all polygons with even number of edges')
print([p.area for p in mypoly if p.count_edges % 2 == 0])

even_area = list([p.area for p in mypoly if p.count_edges % 2 == 0])
odd_area = list([p.area for p in mypoly if p.count_edges % 2 == 1])
print(f'Area of polygons with even number of edges :{even_area}')
print(f'Area of polygons with odd number of edges  :{odd_area}')
print(f'area ratios for even to off sides polygons : {[even_area[i]/odd_area[i] for i in range(len(even_area))]}')
for p in mypoly:
    print(f'Polygon with sides : {p.count_edges} has area = {p.area} and max efficiency = {p.max_efficiency}')

# Print only the maximum efficiency
print(f'Maximum efficiency {reduce(max, [p.polygon_efficiency for p in mypoly])}')